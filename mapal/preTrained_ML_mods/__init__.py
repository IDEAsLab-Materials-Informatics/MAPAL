...
def run_model_M1(input_alloys, input_contains_concentration=False):
    ...
    for mod in sorted(model_list):
        ...
        feat_names_current_mod = df_mod_featnames.loc[mod_id][0].split(",");
        model = models.load_model(f"{mod_files_dir}{mod}"); #loading the tained model into 'model' variable
...
def run_model_M2(input_alloys, input_contains_concentration=False):
    ...
    for mod in sorted(model_list):
        model = models.load_model(f"{mod_files_dir}{mod}");
